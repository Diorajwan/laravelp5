                ...
                <tbody>
                  ...
                  @foreach ($dosen as $data)
                    <tr>
                      ...
                      <td>{{ $data->jabatan }}</td>
                      <td>
                        <img src="{{ asset('storage/dosen/' . $data->foto) }}" alt="dosen Foto" style="width: 50px; height: 50px;">
                      </td>
                      <td>{{ $data->status }}</td>
                      <td>
                        <a href="{{ route('dosen.show', $data->id) }}" class="btn btn-warning btn-sm"><i class="fas fa-eye"></i></a>
                        <a href="{{ route('dosen.edit', $data->id) }}" class="btn btn-info btn-sm"><i class="fas fa-edit"></i></a>
                        <form action="{{ route('dosen.destroy', $data->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                          @csrf
                          @method('DELETE')
