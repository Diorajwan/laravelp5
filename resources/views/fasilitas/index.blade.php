                ...
                <tbody>
                  ...
                  @foreach ($fasilitas as $data)
                    <tr>
                      ...
                      <td><img src="{{ asset('storage/fasilitas/' . $data->foto) }}" alt="" style="width: 50px; height: 50px;"></td>
                      <td>
                        <a href="{{ route('fasilitas.show', $data->id) }}" class="btn btn-warning btn-sm"><i class="fas fa-eye"></i></a>
                        <a href="{{ route('fasilitas.edit', $data->id) }}" class="btn btn-info btn-sm"><i class="fas fa-edit"></i></a>
                        <form action="{{ route('fasilitas.destroy', $data->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                          @csrf
                          @method('DELETE')
